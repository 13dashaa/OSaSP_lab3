...
void resume_processes(int index);
...
{
    printf("\n");
    printf("input one of chars: \n\t+ (cerate new child)\n\t- (delete last child)\n\tm (show menu)");
    printf("\n\tl (show all children info)\n\tk (delete all processes)\n\ts (stop process)\n\tg (resume process)\n\tq (complete the program)\n");
    printf("or input s<num> g<num>, where num - index of process\n");
...
void init_signals_handling()
{
    ...
    action.sa_sigaction = signal_handler;      // установка обработчика сигналов
...
void signal_handler(int signo, siginfo_t *info, void *context)
{
    ...
    else if(signo == SIGALRM) resume_processes(-1);
}
